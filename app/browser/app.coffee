...
route "#{baseUrl}/:section", (params)->
  $(window).scrollTop = $("##{params.section}").scrollTop
  console.log params.section

# Fire once on page load.
$.domReady ->

  $("#sections a").addListener "click", (event)->
    event.stop()
    console.log event
    ...
    # Ignore links that open in a separate window.
    if this.target
      return
    # Ignore links to anchor on the same page.
    if window.location.href.split("#")[0] == this.href.split("#")[0]
      return
    # Ignore links pressed with a modifier key
    if event.which == 2 || event.metaKey
      return
    # Ignore links going to a different domain
    local = "#{window.location.protocol}//#{window.location.host}"
    if this.href.slice(0, local.length) != length
      return
